      ...
      entityCountCmds: [
        {
          ...
          query: { entityFilter: { type: "entityType", entityType: EntityType.TENANT, resolveMultiple: true, } }
        },
        {
          cmdId: TENANT_PROFILE_COUNT_CMD_ID.value,
          query: { entityFilter: { type: "entityType", entityType: EntityType.TENANT_PROFILE, resolveMultiple: true, } }
        },
        {
          cmdId: DEVICE_COUNT_CMD_ID.value,
          query: { entityFilter: { type: "entityType", entityType: EntityType.DEVICE, resolveMultiple: true, } }
        },
        {
          cmdId: ASSET_COUNT_CMD_ID.value,
          query: { entityFilter: { type: "entityType", entityType: EntityType.ASSET, resolveMultiple: true, } }
        },
        {
          cmdId: USER_COUNT_CMD_ID.value,
          query: { entityFilter: { type: "entityType", entityType: EntityType.USER, resolveMultiple: true, } }
        },
        {
          cmdId: CUSTOMER_COUNT_CMD_ID.value,
          query: { entityFilter: { type: "entityType", entityType: EntityType.CUSTOMER, resolveMultiple: true, } }
        }
      ],
      entityDataCmds: [{
...
function unsubscribeEntityCount() {
  const entityCountUnsubscribeCmds: Array<any> = [];
  const entityDataUnsubscribeCmds: Array<any> = [];
  if (TENANT_COUNT_CMD_ID.value > 0) {
    entityCountUnsubscribeCmds.push({ cmdId: TENANT_COUNT_CMD_ID.value });
  }
  if (TENANT_PROFILE_COUNT_CMD_ID.value > 0) {
    entityCountUnsubscribeCmds.push({ cmdId: TENANT_PROFILE_COUNT_CMD_ID.value });
  }
  if (DEVICE_COUNT_CMD_ID.value > 0) {
    entityCountUnsubscribeCmds.push({ cmdId: DEVICE_COUNT_CMD_ID.value });
  }
  if (ASSET_COUNT_CMD_ID.value > 0) {
    entityCountUnsubscribeCmds.push({ cmdId: ASSET_COUNT_CMD_ID.value });
  }
  if (USER_COUNT_CMD_ID.value > 0) {
    entityCountUnsubscribeCmds.push({ cmdId: USER_COUNT_CMD_ID.value });
  }
  if (CUSTOMER_COUNT_CMD_ID.value > 0) {
    entityCountUnsubscribeCmds.push({ cmdId: CUSTOMER_COUNT_CMD_ID.value });
  }
  if (CPU_STATUS_CMD_ID.value > 0) {
    entityDataUnsubscribeCmds.push({ cmdId: CPU_STATUS_CMD_ID.value });
  }
  unsubscribe(
    [TENANT_COUNT_CMD_ID.value, TENANT_PROFILE_COUNT_CMD_ID.value, DEVICE_COUNT_CMD_ID.value, ASSET_COUNT_CMD_ID.value, USER_COUNT_CMD_ID.value, CUSTOMER_COUNT_CMD_ID.value],
    { entityCountUnsubscribeCmds: entityCountUnsubscribeCmds, entityDataUnsubscribeCmds: entityDataUnsubscribeCmds }
  )
}
